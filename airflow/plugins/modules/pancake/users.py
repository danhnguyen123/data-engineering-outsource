...
class UsersETL:
    ...
    def extract(self):
        ...
        self.logger.debug(f"Get data {self.table_name} from Pancake | page {page_name} | page {platform}")
            
        results = self.pancake.get_users(
            page_access_token=page_access_token,
            page_id=page_id,
        )

        active_users = results.get("users")
        disabled_users = results.get("disabled_users")
        ...
        df = pd.DataFrame.from_records(user_rows)
